    ...
    if not uploaded_zip_file:
        ...
        try:
            with st.spinner("Baixando os modelos..."):
                # NOVO: Uso da biblioteca gdown para download
                gdown.download(id=MODEL_F1_ID, output=path_modelo_f1, quiet=False)
                gdown.download(id=MODEL_F2_ID, output=path_modelo_f2, quiet=False)
