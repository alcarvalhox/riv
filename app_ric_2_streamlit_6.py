...
def download_gdrive_file(file_id, destination):
    """
    Baixa um arquivo do Google Drive usando uma URL de download direto.
    """
    URL = f"https://drive.google.com/uc?id={file_id}&export=download"
    response = requests.get(URL, stream=True)
    
    if "Download quota exceeded" in response.text:
        st.error("Erro: A cota de download do Google Drive foi excedida. Tente novamente mais tarde.")
        return False
...
# Função para encontrar o diretório das imagens
...
# --- FUNÇÃO PRINCIPAL run_yolo_predictions COM VERIFICAÇÃO ADICIONAL ---
def run_yolo_predictions(path_modelo_f1, path_modelo_f2, src_dir, path_res, pasta_inferencia, arq_inferencia):
    ...
    with st.spinner('Executando a inferência YOLO...'):
        try:
            ...
            # NOVO: Verificação de arquivos de imagem válidos antes da inferência
            image_files = [os.path.join(source_directory, f) for f in os.listdir(source_directory) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp', '.bmp'))]
            if not image_files:
                return "Erro: O diretório de imagens encontrado está vazio ou não contém arquivos de imagem válidos."
    ...
    if not uploaded_zip_file:
        ...
        try:
            with st.spinner("Baixando os modelos..."):
                if not download_gdrive_file(MODEL_F1_ID, path_modelo_f1):
                    st.stop()
                if not download_gdrive_file(MODEL_F2_ID, path_modelo_f2):
                    st.stop()
